...
select yn in "Y" "N"; do
    case $yn in
        Y ) sirius --config config/sirius-compose.yaml credentials issue agent-0 --subject indy-wallet --schema-name TEST1 --comment "this is a test" --attr Name=Phil --attr City=Durham;;
        N ) exit;;
    esac
done
